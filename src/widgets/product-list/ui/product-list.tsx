...
import { EditDeletePanel } from '@/shared/ui';
import { colors } from '@/shared/styles/global';
import EditProductModal from '@/features/edit-product'; // Исправляем импорт
import { deleteProduct } from '@/entities/product/model/product-slice';
import ConfirmDeleteModal from '@/features/delete-product'; // Исправляем импорт
...
export const ProductList = () => {
  ...
  const products = useSelector(selectFilteredSortedSearchedProducts); // Используем селектор
  const [selectedProduct, setSelectedProduct] = useState<Product | null>(null);
  const [isEditDeletePanelVisible, setEditDeletePanelVisible] = useState(false);
  const [isEditModalVisible, setEditModalVisible] = useState(false);
  const [isDeleteModalVisible, setDeleteModalVisible] = useState(false);
  ...
  const handleConfirmDelete = () => {
    if (selectedProduct && user) {
      dispatch(deleteProduct(selectedProduct.id, user.id)); // Передаем userId
    }
    setDeleteModalVisible(false);
    setSelectedProduct(null);
    setEditDeletePanelVisible(false);
